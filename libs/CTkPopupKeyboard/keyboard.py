...
class PopupKeyboard(CTkToplevel):
    #keyboard.frame.configure(width=322, height=130)
    ...
    def _iconify(self):
        ...
        if self.hide:
            ...
            #keyboard.frame.configure(width=322, height=130)
            #self.geometry('{}x{}+{}+{}'.format(self.frame.winfo_reqwidth(),self.frame.winfo_reqheight(),self.x_pos,self.y_pos))
            # Calcola le dimensioni massime del frame
            max_width = max([len(row) for row in self.keys.values()]) * self.keywidth
            max_height = 5 * self.keyheight
            print("max_width: " + str(max_width))
            print("max_height: " + str(max_height))
            print("self.keywidth: " + str(self.keywidth))
            

            # Imposta la dimensione della finestra in base alle dimensioni massime del frame
            #self.geometry('{}x{}+{}+{}'.format(max_width, max_height, self.x_pos, self.y_pos))
